...
public class TeacherProfile extends javax.swing.JDialog {
    ...
    private void loadState() {
        String qurty = "SELECT `id`,`name` FROM `teacher_status`";
        try {
            stetusMap = ItemLoader.getItemLoader().loadComboPlus(status, qurty, new String[0]);
        } catch (ClassNotFoundException | SQLException | IOException ex) {
            LogWritter.logger.log(Level.WARNING, "Teacher Profile Status Loading", ex);
        }
    }
    ...
    private void findTeacher() {
        try {
            ResultSet rs = DB.search("SELECT CONCAT(`fname`,'',`lname`), `teacher_status`.`name`,"
                    ...
                    + "`city`.`name` FROM `teacher` "
                    + "INNER JOIN `teacher_status` ON `teacher_status`.`id` = `teacher`.`teacher_status_id` "
